...
public abstract class TelaBase<Tipo> where Tipo : EntidadeBase<Tipo>
{
    ...
    public virtual char ApresentarMenu()
    {
        ...
        Console.WriteLine($"2 - Editar {nomeEntidade}", ConsoleColor.Yellow);
        Console.WriteLine($"3 - Excluir {nomeEntidade}", ConsoleColor.Red);
        Console.WriteLine($"4 - Visualizar {nomeEntidade}s");
        Console.WriteLine($"S - Sair", ConsoleColor.White);

        Console.WriteLine();

        Console.Write("Digite uma opção válida: ", ConsoleColor.Red);
